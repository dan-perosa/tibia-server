...
function XPAnalyser:setupLevel(level, percent)
	...
	XPAnalyser.window.contentsPanel.percent:setPercent(math.floor(percent / 100))
...
function XPAnalyser:updateExpensiveUI()
	...
	if player then
		contentsPanel.percent:setPercent(math.floor(player:getLevelPercent() / 100))
	else
...
function XPAnalyser:updateTooltip()
	...
	text = text .. "\nYou have " .. (100 - player:getLevelPercent() / 100) .. " percent to go."
