...
function refresh()
    ...
    -- Percent bars (magic level AND regular weapon skills) don't reliably get a live update
    -- signal in this build -- confirmed 2026-08-25 for magic level, and 2026-08-26 the same
    -- symptom (bar never visibly progresses, even though the underlying level DOES advance
    -- eventually) was reported for every skill, not just magic level. Poll all of them directly
    -- as a targeted workaround instead of relying on the change event.
    if magicPercentRefreshEvent then
        magicPercentRefreshEvent:cancel()
        magicPercentRefreshEvent = nil
    end
    magicPercentRefreshEvent = cycleEvent(function()
        local localPlayer = g_game.getLocalPlayer()
        if not localPlayer then
            return
        end
        onMagicLevelChange(localPlayer, localPlayer:getMagicLevel(), localPlayer:getMagicLevelPercent())
        onLevelChange(localPlayer, localPlayer:getLevel(), localPlayer:getLevelPercent())
        for i = Skill.Fist, Skill.Transcendence do
            onSkillChange(localPlayer, i, localPlayer:getSkillLevel(i), localPlayer:getSkillLevelPercent(i))
        end
    end, 1000)
...
function onLevelChange(localPlayer, value, percent)
    ...
    percent = (percent or localPlayer:getLevelPercent()) / 100
...
function onMagicLevelChange(localPlayer, magiclevel, percent)
    percent = (percent or localPlayer:getMagicLevelPercent()) / 100
...
function onSkillChange(localPlayer, id, level, percent)
    percent = (percent or localPlayer:getSkillLevelPercent(id)) / 100
