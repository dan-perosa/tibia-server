    ...
    if player then
        ...
        if not table.empty(lastCombatControls) then
            if lastCombatControls[char] then
                ...
                -- onChaseModeChange/onFightModeChange aren't connected yet at this point
                -- (that happens further down in this same function), so the restored modes
                -- never reach the UI -- the posture/combat buttons stay showing "stand"/default
                -- even when chase mode (etc.) actually got restored to on. Force a resync.
                combatEvent()
            end
        end
    end
